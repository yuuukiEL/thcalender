...
document.addEventListener('DOMContentLoaded', function() {
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (files.length > 0) {
                ...
                reader.onload = function(e) {
                    ...
                    console.log(`Image state updated for button ${currentActiveButton}`);
        ...
        try {
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                if (imageState && imageState.file) {
                    console.log(`Adding image ${i}:`, imageState.file.name);
                    formData.append(`images[]`, imageState.file, imageState.file.name);
                }
            }
            ...
            // サーバーへのデータ送信
            const response = await fetch('/student/diary/api/save', {
                method: 'POST',
                body: formData
            });
            ...
            // テキストレスポンスの場合も成功として扱う
            const contentType = response.headers.get('content-type');
            if (contentType && contentType.includes('text/html')) {
                updateSaveButtonState('success');
                alert('日記が保存されました');
                setTimeout(() => {
                    window.location.href = '/student/diary';
                }, 1000);
                return;
            }
    ...
    function initializeHashtagFeature() {
        ...
        function addHashtag() {
            ...
            if (tagText) {
                ...
                if (hashtagContainer) {
                    ...
                    listHashtagSpan.style.cursor = 'pointer';  // カーソルをポインターに
                    
                    // ダブルクリックで直接削除
                    listHashtagSpan.addEventListener('dblclick', function() {
                        this.remove();
                        
                        // 本文中の対応するハッシュタグも削除
                        const contentHashtags = diaryContent.querySelectorAll('.hashtag');
        ...
        function initializeExistingHashtags() {
            const hashtagContainer = document.getElementById('hashtagContainer');
            if (hashtagContainer) {
                const existingHashtags = hashtagContainer.querySelectorAll('.hashtag');
                existingHashtags.forEach(tag => {
                    ...
                    tag.addEventListener('dblclick', function() {
                        ...
                        // 本文中の対応するハッシュタグも削除
                        const contentHashtags = diaryContent.querySelectorAll('.hashtag');
                        contentHashtags.forEach(contentTag => {
                            if (contentTag.textContent === tagText) {
                                contentTag.remove();
                            }
                        });
                    });
        ...
        // DOMContentLoadedイベントで既存のハッシュタグを初期化
        document.addEventListener('DOMContentLoaded', function() {
    ...
    if (diaryContent) {
        ...
        diaryContent.addEventListener('contextmenu', function (e) {
            ...
            if (selection.toString().length > 0) {
                ...
                // メニューを右側に表示（現在位置から水平に）
                const menuWidth = contextMenu.offsetWidth;
                const menuHeight = contextMenu.offsetHeight;
                const x = e.pageX + 10; // 現在位置から10px右に
                const y = e.pageY;
